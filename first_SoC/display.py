...
class SevenSegmentDisplay0(Module, AutoCSR):
    def __init__(self, sys_clk_freq):
        ...
        self.sync += [
            ...
            If(self.write.re,
                ...
                If(self.sel.storage==0, 
                    display.values.eq(self.value.storage)                    
                )
            )
        ]
class SevenSegmentDisplay1(Module, AutoCSR):
    def __init__(self, sys_clk_freq):
        self.sel   = CSRStorage(4)
        self.value = CSRStorage(4)
        self.write = CSR()
        ...
        self.sync += [
            ...
            If(self.write.re,
                # Select witch value to update based on sel register
                If(self.sel.storage==1, 
                    display.values.eq(self.value.storage)                    
                )
            )
        ]        
class SevenSegmentDisplay2(Module, AutoCSR):
    def __init__(self, sys_clk_freq):
        self.sel   = CSRStorage(4)
        self.value = CSRStorage(4)
        self.write = CSR()
        ...
        self.sync += [
            ...
            If(self.write.re,
                # Select witch value to update based on sel register
                If(self.sel.storage==2, 
                    display.values.eq(self.value.storage)                    
                )
            )
        ]
class SevenSegmentDisplay3(Module, AutoCSR):
    def __init__(self, sys_clk_freq):
        self.sel   = CSRStorage(4)
        self.value = CSRStorage(4)
        self.write = CSR()
        ...
        self.sync += [
            ...
            If(self.write.re,
                # Select witch value to update based on sel register
                If(self.sel.storage==3, 
                    display.values.eq(self.value.storage)                    
                )
            )
        ]
class SevenSegmentDisplay4(Module, AutoCSR):
    def __init__(self, sys_clk_freq):
        self.sel   = CSRStorage(4)
        self.value = CSRStorage(4)
        self.write = CSR()
        ...
        self.sync += [
            ...
            If(self.write.re,
                # Select witch value to update based on sel register
                If(self.sel.storage==4, 
                    display.values.eq(self.value.storage)                    
                )
            )
        ]  
class SevenSegmentDisplay5(Module, AutoCSR):
    def __init__(self, sys_clk_freq):
        self.sel   = CSRStorage(4)
        self.value = CSRStorage(4)
        self.write = CSR()
